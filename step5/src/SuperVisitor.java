		...
		for (Scope scope : table.scopestack.subList(0, table.scopestack.size())) {
			...
			if (scope.Scopetype.equalsIgnoreCase("GLOBAL")) {
				for (String key : scope.symbolMap.keySet()) {
					...
					} else if (((Symbol) scope.symbolMap.get(key)).getType()
							.contains("FLOAT")) {
	...
	public Node findIdNode(String id, String scopeName) {
		if ((this.tableMap.get(scopeName)).get(id) == null) {
			if ((this.tableMap.get("GLOBAL")).get(id) == null) {
				System.out
						.println("can't find assign variable, error@findIdNode");
				return null;
			}
	...
	public Node visitCond(@NotNull LexParser.CondContext ctx) {
        ...
        if (exprnode.type == 1){
		    if (comptype.equalsIgnoreCase("<")) {
			  return new Node("GEI " + exprnode.content + " " + exprnode1.content,
					4);
		    }
		   if (comptype.equalsIgnoreCase(">")) {
			 return new Node("LEI " + exprnode.content + " " + exprnode1.content,
					4);
		   }
		  if (comptype.equalsIgnoreCase("=")) {
			 return new Node("NEI " + exprnode.content + " " + exprnode1.content,
					4);
		   }
		  if (comptype.equalsIgnoreCase("!=")) {
			return new Node("EQI " + exprnode.content + " " + exprnode1.content,
					4);
		   }
		 if (comptype.equalsIgnoreCase("<=")) {
			 return new Node("GTI " + exprnode.content + " " + exprnode1.content,
					4);
		  }
		 if (comptype.equalsIgnoreCase(">=")) {
			 return new Node("LTI " + exprnode.content + " " + exprnode1.content,
					4);
		  }
       }
       else{
		    if (comptype.equalsIgnoreCase("<")) {
			  return new Node("GEF " + exprnode.content + " " + exprnode1.content,
					4);
		    }
		   if (comptype.equalsIgnoreCase(">")) {
			 return new Node("LEF " + exprnode.content + " " + exprnode1.content,
					4);
		   }
		  if (comptype.equalsIgnoreCase("=")) {
			 return new Node("NEF " + exprnode.content + " " + exprnode1.content,
					4);
		   }
		  if (comptype.equalsIgnoreCase("!=")) {
			return new Node("EQF " + exprnode.content + " " + exprnode1.content,
					4);
		   }
		 if (comptype.equalsIgnoreCase("<=")) {
			 return new Node("GTF " + exprnode.content + " " + exprnode1.content,
					4);
		  }
		 if (comptype.equalsIgnoreCase(">=")) {
			 return new Node("LTF " + exprnode.content + " " + exprnode1.content,
					4);
		  }
...
*/
	...
	public String createTemp() {
		...
		(this.tempMap.get(this.functionRecord)).add("$T"
