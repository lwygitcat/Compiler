...
//private ArrayList<Node> inexpr = new ArrayList<Node>();
  ...
  public Node visitPrimary(@NotNull LexParser.PrimaryContext ctx) 
  {
    ...
    if (ctx.INTLITERAL() != null) {
      ...
      outputList.add("STOREI " + ctx.INTLITERAL().getText() + " " + newNode.content); 

      return newNode;
    }

    Node newNode = new Node(createTemp(), 2);
    outputList.add("STOREF " + ctx.FLOATLITERAL().getText() + " " + newNode.content);
...
//'WRITE' '(' id_list ')' ';'
  public Node visitWrite_stmt(@NotNull LexParser.Write_stmtContext ctx)
  {
    ...
    for (int i = 0; i < idArray.length; i++) {
      ...
      if (newNode.type == 1) {
        outputList.add("WRITEI " + newNode.content);
      }
      else if (newNode.type == 5) {
        outputList.add("WRITES " + newNode.content);
      }
      else
      {
        outputList.add("WRITEF " + newNode.content);
      }
    }
  ...
  public Node visitRead_stmt(@NotNull LexParser.Read_stmtContext ctx)
  {
    ...
    for (int i = 0; i < idArray.length; i++) {
      Node newNode = findIdNode(idArray[i], this.functionRecord);
      if (newNode.type == 1) {
        outputList.add("READI " + newNode.content);
      }
      else
      {
        outputList.add("READF " + newNode.content);
      }
    }
  ...
  public Node visitAssign_expr(@NotNull LexParser.Assign_exprContext ctx)
  {
    ...
    if (newNode.type == 1) {
      outputList.add("STOREI " + exprNode.content + " " + newNode.content);
    }
    else
    {
      outputList.add("STOREF " + exprNode.content + " " + newNode.content);
    }
...
//clear  
  ...
  public String return3AC() {
    ...
    result = result + ";"+(String)outputList.get(outputList.size()-1);
